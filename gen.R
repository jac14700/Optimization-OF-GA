fitnessFunc<-function(chromosome)
{
  ...
  for (GEN_INDEX in 1:(n)) {
    firstSum <- firstSum+ chromosome[GEN_INDEX] ^2.0;
    secondSum<- secondSum+ cos(2.0*pi*chromosome[GEN_INDEX]);
  }
  return (10-(-20.0*exp(-0.2*sqrt(firstSum/n)) - exp(secondSum/n) + 20 + exp(1)))
}
  ...
  for (Generation in 1:1000)
    {
      ...
      cat("bestFitness:",Best_fitness[length(Best_fitness)],"\nAvg_fitness:",Avg_fitness[length(Avg_fitness)],"\n\n")
      #輪盤法
      roulette<-data.frame(matrix(NA,1,population))
      ...
      if(Generation>20 & (no_disterb>20))
      {
        ...
        {
          drop.out<-order(next_gen[,Gene_length+1],decreasing=FALSE)[1:round(population*.666667,.1)]
...
# setwd("E:/School stuffs/自傳履歷/Gen AG")

write.csv(cbind(Best_fitness,Worst_fitness,Avg_fitness,fitness_std),paste0("P",population," C",Gene_length,".csv"))

apple$Best_fitness

apple <- read.csv("P100 C40.csv")
no_disterb_Avg <- read.csv("Avg_fitness.csv")

Generation<-c(1:75)
Fitness<-no_disterb_Best$x
Avg_fitness<-no_disterb_Avg$x
plot1<-plot(Fitness~Generation,type="l",col="blue")
lines(Avg_fitness~Generation, col="red")
legend("topleft",c("Best_fitness","Avg_fitness"), col=c("blue","red"), y.intersp=1.5,lty=1:1, cex=0.8)
title("Genetic Algorithm: Ackley Function")
